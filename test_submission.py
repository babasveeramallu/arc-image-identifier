...
def test_depth_estimation():
    ...
    try:
        ...
        print("  OK: Depth estimation works")
        return True
...
def test_dual_detection():
    ...
    try:
        ...
        print("  OK: Dual detection initialized")
        
        # Test detection
        test_img = np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8)
        results = detector.detect_comprehensive(test_img)
        
        print(f"  OK: Detection works (found {results['total_objects']} objects in test image)")
        return True
        
    except Exception as e:
        print(f"  ERROR: Dual detection failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_scanner():
    ...
    try:
        ...
        print("  OK: Scanner components initialized")
        
        # Test depth estimation
        test_img = np.random.randint(0, 255, (480, 640, 3), dtype=np.uint8)
        depth = depth_est.estimate(test_img)
        
        print(f"  OK: Depth map generated: {depth.shape}")
        
        # Test point cloud
        pcd = pcd_proc.depth_to_pointcloud(depth, test_img)
        print(f"  OK: Point cloud generated: {len(pcd.points)} points")
        
        return True
        
    except Exception as e:
        print(f"  ERROR: Scanner test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_camera():
    ...
    try:
        ...
        if not cap.isOpened():
            print("  WARNING: Camera not available (OK for demo without camera)")
            return True
        ...
        if ret:
            print(f"  OK: Camera works: {frame.shape}")
            return True
        else:
            print("  WARNING: Camera opened but couldn't read frame")
            return True
            
    except Exception as e:
        print(f"  WARNING: Camera test failed: {e} (OK for static demo)")
        return True
